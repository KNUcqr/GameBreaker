...
local Room = workspace.Room1

local Start = Room.Destination

local Already = {true,true,true,true,true,true}
...
local function onStartTouched(hit) -- 플레이어가 시작 파트에 닿았을 때
	if Triggered then
		...
		if character and character:FindFirstChild("Humanoid") then
			...
			if player then
				remote:FireClient(player, 1200, "계산해라")  -- 리모트 이벤트 실행
			end
			
			Triggered = false
			
			-- 버튼 한번만 실행 위한 변수
			Already = {true,true,true,true,true,true}
